...
def index(request, field=None, value=None, tmpl='comment/index.html'):
...
@login_required
def delete(request, safekey, tmpl='snippets/confirm.html'):
    obj = ndb.Key(urlsafe=safekey).get()
    if users.get_current_user() != obj.author:
